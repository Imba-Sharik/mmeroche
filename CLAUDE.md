...
- Полноэкранные полупрозрачные слои браузер композитит по отдельности и режет
  на тайлы — на стыке видны прямоугольные швы. Особенно заметно с
  `mix-blend-*`, а рядом стоящий элемент с `filter` (его GSAP ставит на время
  появления) создаёт свой контекст наложения и добавляет ещё один шов.
  Поэтому туман Hero рисуется в один `<canvas>` (`HeroFog`), а фон первого
  экрана держится в своём контексте через `isolate`.
...
- `Hero` — содержимое проявляется из размытия (`HeroReveal`, обёртка с
  `display: contents`, виджет остаётся серверным), поверх фона — дым на холсте
  (`HeroFog`): медленный ход по синусоиде плюс отклик на курсор с запаздыванием.
...
есть в `Hero`, `Kitchen`, `Spaces` и `Story`, в «Контактах» живая карта;
страниц меню и бара пока нет.
...
- туманы остальных секций: из макета взят только слой Hero («59 1»), не хватает
  «17 1» (`114:2447`), «89 1» (`114:2609`) и «16 1» (`114:2606`) — забрать,
  когда будет доступ к Figma.
